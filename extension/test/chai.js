                ...
                module.exports = function (chai, _) {
                    ...
                    function include(val, msg) {
                        if (msg)
                            flag(this, 'message', msg);
                        var obj = flag(this, 'object');
                        var objType = _.type(obj).toLowerCase();
                        var flagMsg = flag(this, 'message');
                        var negate = flag(this, 'negate');
                        var ssfi = flag(this, 'ssfi');
                        var isDeep = flag(this, 'deep');
                        var descriptor = isDeep ? 'deep ' : '';
                    ...
                    function assertAbove(n, msg) {
                        if (msg)
                            flag(this, 'message', msg);
                        var obj = flag(this, 'object');
                        var doLength = flag(this, 'doLength');
                        var flagMsg = flag(this, 'message');
                        var msgPrefix = flagMsg ? flagMsg + ': ' : '';
                        var ssfi = flag(this, 'ssfi');
                        var objType = _.type(obj).toLowerCase();
                        var nType = _.type(n).toLowerCase();
                    ...
                    Assertion.addMethod('within', function (start, finish, msg) {
                        ...
                        var ssfi = flag(this, 'ssfi');
                        var objType = _.type(obj).toLowerCase();
                        var startType = _.type(start).toLowerCase();
                        var finishType = _.type(finish).toLowerCase();
                        var shouldThrow = true;
                        var range = startType === 'date' && finishType === 'date'
                    ...
                    function assertProperty(name, val, msg) {
                        ...
                        var isDeep = flag(this, 'deep');
                        var negate = flag(this, 'negate');
                        var pathInfo = isNested ? _.getPathInfo(obj, name) : null;
                        var value = isNested ? pathInfo.value : obj[name];
                        var descriptor = '';
                        if (isDeep)
                            descriptor += 'deep ';
                        if (isOwn)
                    ...
                    function assertOwnPropertyDescriptor(name, descriptor, msg) {
                        ...
                        if (actualDescriptor && descriptor) {
                            this.assert(_.eql(descriptor, actualDescriptor), 'expected the own property descriptor for ' +
                                _.inspect(name) +
                                ' on #{this} to match ' +
                                _.inspect(descriptor) +
                                ', got ' +
                                _.inspect(actualDescriptor), 'expected the own property descriptor for ' +
                                _.inspect(name) +
                                ' on #{this} to not match ' +
                                _.inspect(descriptor), descriptor, actualDescriptor, true);
                        }
                        else {
                    ...
                    function assertKeys(keys) {
                        var obj = flag(this, 'object');
                        var objType = _.type(obj);
                        var keysType = _.type(keys);
                        var ssfi = flag(this, 'ssfi');
                        var isDeep = flag(this, 'deep');
                        var str;
                        var deepStr = '';
                        ...
                        if (errorLike && caughtErr) {
                            ...
                            if (isCompatibleConstructor === negate) {
                                if (everyArgIsDefined && negate) {
                                    ...
                                    this.assert(negate, 'expected #{this} to throw #{exp} but #{act} was thrown', 'expected #{this} to not throw #{exp}' + (caughtErr ? ' but #{act} was thrown' : ''), errorLike instanceof Error
                                        ? errorLike.toString()
                                        : errorLike && _.checkError.getConstructorName(errorLike), caughtErr instanceof Error
                                        ? caughtErr.toString()
                                        : caughtErr && _.checkError.getConstructorName(caughtErr));
                                }
                            }
                    ...
                    function respondTo(method, msg) {
                        ...
                        var context = typeof obj === 'function' && !itself ? obj.prototype[method] : obj[method];
            ...
            function (require, module, exports) {
                ...
                module.exports = function (chai, util) {
                    ...
                    assert.operator = function (val, operator, val2, msg) {
                        ...
                        test.assert(flag(test, 'object') === true, 'expected ' + util.inspect(val) + ' to be ' + operator + ' ' + util.inspect(val2), 'expected ' + util.inspect(val) + ' to not be ' + operator + ' ' + util.inspect(val2));
                    };
                    /**
                ...
                function stringDistance(strA, strB, memo) {
                    ...
                    if (!memo[strA.length] || !memo[strA.length][strB.length]) {
                        if (strA.length === 0 || strB.length === 0) {
                            ...
                            memo[strA.length][strB.length] = Math.min(stringDistance(strA.slice(0, -1), strB, memo) + 1, stringDistance(strA, strB.slice(0, -1), memo) + 1, stringDistance(strA.slice(0, -1), strB.slice(0, -1), memo) +
