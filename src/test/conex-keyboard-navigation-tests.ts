...
import {fillContainer, renderContainers} from '../conex-containers.js';
...
describe('keyboard navigation', function () {
    ...
    it('should react on down and up arrow keys for container elements with tabs correctly', async function () {
        ...
        for (const keys of [
            ...
            // Test 2
            typeKey(keys.down, document.activeElement!);
            expect(e2t(document.activeElement! as HTMLElement)).to.equal(e2t($('ul>li:nth-child(1)', containerElements[1]!)!));

            // one arrow down:  we should now be on the second tab within the first container
            // Test 3
            typeKey(keys.down, document.activeElement!);
            expect(e2t(document.activeElement! as HTMLElement)).to.equal(e2t($('ul>li:nth-child(2)', containerElements[1]!)!));
            ...
            // Test 5
            typeKey(keys.down, document.activeElement!);
            expect(e2t(document.activeElement! as HTMLElement)).to.equal(e2t($('ul>li:nth-child(2)', containerElements[2]!)!));
            ...
            // Test 8
            typeKey(keys.down, document.activeElement!);
            expect(e2t(document.activeElement! as HTMLElement)).to.equal(e2t($('ul>li:nth-child(1)', containerElements[5]!)!));
    ...
    it('should select search term when returning to search field', async function () {
        await renderContainers(fakeContainers);
        ($('#search')! as HTMLInputElement).value = 'fake';
        $('ol > li')!.focus();
        // containerElements[0].focus();
        typeKey({key: 'ArrowUp'}, document.activeElement!);
        expect(document.activeElement!).to.equal($('#search'));
        ...
        expect(($('#search')! as HTMLInputElement).selectionStart).to.equal(0);
        expect(($('#search')! as HTMLInputElement).selectionEnd).to.equal(4);
    });
});
